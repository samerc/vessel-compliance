...
import {
  Building2, User, Shield, ShieldCheck, ShieldAlert, RefreshCw, Loader2, Pencil,
  Save, Trash2, Upload, FolderOpen, Plus, CheckCircle2, AlertTriangle, MapPin, UserCheck, Mail, Phone, Hash
} from 'lucide-react'
import { Entity, EntityUBO, SanctionsMatch, EntityAddress, EntityDocumentType, EntityDocument } from '../../../shared/types'
 ...
 */
export default function EntityEditPanel({ entityId, canManage, onChanged, showCommissions = true }: EntityEditPanelProps) {
  ...
  const [commissions, setCommissions] = useState<{ policyTypeId: string; commissionPercent: number }[]>([])
  ...
  const [form, setForm] = useState<{ name: string; type: 'company' | 'person'; identifier: string; email: string; phone: string }>({ name: '', type: 'company', identifier: '', email: '', phone: '' })
  const [savingCore, setSavingCore] = useState(false)

  // ── Sanctions ───────────────────────────────────────────────────────────────────
  const [checkingId, setCheckingId] = useState<string | null>(null)
  const [sanctionsModal, setSanctionsModal] = useState<{ show: boolean; searchedName: string; matches: SanctionsMatch[]; entityId?: string }>({ show: false, searchedName: '', matches: [] })
  ...
  const loadEntityData = async () => {
    ...
    if (showCommissions) {
      try {
        ...
        setCommissions(Array.isArray(co) ? co.map((o: any) => ({ policyTypeId: o.policyTypeId, commissionPercent: o.commissionPercent })) : [])
        if (Array.isArray(pt)) setPolicyTypes(pt.map((t: any) => ({ id: t.id, name: t.name, code: t.code })))
        if (Array.isArray(cd)) { const m: Record<string, number> = {}; for (const d of cd as any[]) m[d.policyTypeId] = d.commissionPercent; setCommDefaults(m) }
      } catch { /* ignore */ }
    }
  }
  ...
  const refresh = () => { loadEntityData(); onChanged() }

  // ── Core field handlers ─────────────────────────────────────────────────────────
  const startEditCore = () => {
    if (!entity) return
    setForm({ name: entity.name, type: entity.type, identifier: entity.identifier || '', email: entity.email || '', phone: entity.phone || '' })
    setEditingCore(true)
  }
  ...
  const handleOfacRecheck = async (ent: Entity) => {
    ...
    try {
      ...
      if (result.status !== 'CLEARED' || autoMark) {
        await window.api.updateEntity(ent.id, { ofacCheckedAt: result.timestamp, ofacMatchFound: result.matchFound, ofacStatus: result.status })
        refresh()
      } else {
        showSuccess('Sanctions check complete: no matches found above threshold')
      }
      if (result.matchFound && result.matches.length > 0) {
        setSanctionsModal({ show: true, searchedName: ent.name, matches: result.matches, entityId: ent.id })
      }
    } catch (error: any) {
  ...
  const startEditAddress = (addr: EntityAddress) => {
    ...
    setAddrForm({ label: addr.label, addressLine1: addr.addressLine1, addressLine2: addr.addressLine2 || '', city: addr.city || '', country: addr.country || '', postalCode: addr.postalCode || '' })
  ...
  const handleAddUBO = async () => {
    ...
    try {
      ...
      setNewUBOName(''); setNewUBOType('person'); setNewUBOIdentifier(''); setNewUBOEmail(''); setNewUBOPhone(''); setSelectedUBOId(null)
  ...
  const handleLinkExistingUBO = async (uboEntityId: string) => {
    ...
    try {
      await window.api.addEntityUBO({ assuredEntityId: entity.id, uboEntityId })
      setNewUBOName(''); setSelectedUBOId(null)
  ...
  const handleDeleteUBO = async (uboEntityId: string) => {
    if (!entity) return
    const ok = await confirmDialog('Are you sure you want to remove this UBO?', { title: 'Remove UBO?' })
    if (!ok) return
    try {
  ...
  // ── Derived ─────────────────────────────────────────────────────────────────────
  const selectedUbos = entity ? ubos.filter(u => u.assuredEntityId === entity.id).map(u => entities.find(e => e.id === u.uboEntityId)).filter(Boolean) as Entity[] : []
  const uboLinkedIds = new Set(selectedUbos.map(u => u.id))
  const matchingUBOs = entities.filter(ent => newUBOName && ent.id !== entityId && !uboLinkedIds.has(ent.id) && ent.name.toLowerCase().includes(newUBOName.toLowerCase())).slice(0, 8)

  const getDocScore = (eId: string, entityType: string) => {
    const applicable = docTypes.filter(t => t.isRequired && (t.entityScope === 'both' || t.entityScope === entityType))
    const have = applicable.filter(t => docs.some(d => d.entityId === eId && d.documentTypeId === t.id && d.filePath)).length
  ...
  const OfacBadge = ({ ent }: { ent: Entity }) => {
    ...
    return (
      <div
        style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', padding: '2px 7px', borderRadius: '4px', fontSize: '0.68rem', background: config.bg, border: config.border, color: config.color, cursor: isClickable ? 'pointer' : 'default', whiteSpace: 'nowrap' }}
        title={isError ? 'API request failed. Click refresh to retry.' : isClickable ? 'Click to review matches' : `Last checked: ${ent.ofacCheckedAt ? formatDateTime(ent.ofacCheckedAt) : 'Never'}`}
        onClick={e => { e.stopPropagation(); if (isClickable) handleViewPotentialMatch(ent) }}
      >
        {config.icon}
        {config.text}
        <RefreshCw size={9} style={{ marginLeft: '3px', cursor: 'pointer', opacity: 0.55 }} className="hover-spin" onClick={e => { e.stopPropagation(); handleOfacRecheck(ent) }} />
      </div>
    )
  }

  const DocRow = ({ ent }: { ent: Entity }) => {
    const applicable = docTypes.filter(t => t.entityScope === 'both' || t.entityScope === ent.type)
            ...
            >
              ...
              {hasFile ? (
                <div style={{ display: 'flex', gap: '4px' }}>
                  ...
                  {canUploadDocs && <button onClick={e => { e.stopPropagation(); handleClickUploadDoc(ent.id, dt.id, dt.name) }} className="btn-secondary" style={{ padding: '3px 6px', fontSize: '0.68rem' }} title="Replace"><Upload size={10} /></button>}
                ...
                canUploadDocs && <button onClick={() => handleClickUploadDoc(ent.id, dt.id, dt.name)} className="btn-secondary" style={{ padding: '3px 8px', fontSize: '0.7rem', display: 'flex', alignItems: 'center', gap: '3px' }}><Upload size={10} /> Upload</button>
              )}
            </div>
          )
        })}
        {applicable.length === 0 && <div style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', fontStyle: 'italic' }}>No document types for this entity.</div>}
      </div>
    )
  }
  ...
  const sectionLabel: React.CSSProperties = { fontSize: '0.68rem', textTransform: 'uppercase', letterSpacing: '0.7px', color: 'var(--text-secondary)', fontWeight: 600 }
      ...
      <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--table-border)' }}>
        {editingCore ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <div>
              <label style={{ ...sectionLabel, display: 'block', marginBottom: '4px' }}>Name *</label>
              <div style={{ position: 'relative' }}>
                <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} style={{ width: '100%', paddingRight: '44px' }} placeholder="Entity name" />
                <CaseToggleBtn value={form.name} onChange={v => setForm(f => ({ ...f, name: v }))} />
              </div>
            </div>
            <div style={{ display: 'flex', gap: '10px' }}>
              <div style={{ flex: 1 }}>
                <label style={{ ...sectionLabel, display: 'block', marginBottom: '4px' }}>Type</label>
                <select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value as any }))} style={{ width: '100%', padding: '8px 10px' }}>
                  <option value="company">Company</option>
                  <option value="person">Person</option>
                </select>
              </div>
              <div style={{ flex: 1 }}>
                <label style={{ ...sectionLabel, display: 'block', marginBottom: '4px' }}>Identifier</label>
                <input type="text" value={form.identifier} onChange={e => setForm(f => ({ ...f, identifier: e.target.value }))} style={{ width: '100%' }} placeholder="Optional" />
              </div>
            </div>
            <div>
              <label style={{ ...sectionLabel, display: 'block', marginBottom: '4px' }}>Email(s)</label>
              <input type="text" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} style={{ width: '100%' }} placeholder="Separate multiple with commas" />
            </div>
            <div>
              <label style={{ ...sectionLabel, display: 'block', marginBottom: '4px' }}>Phone</label>
              <input type="text" value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} style={{ width: '100%' }} placeholder="Phone number" />
            </div>
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={handleSaveCore} disabled={!form.name.trim() || savingCore} className="btn-primary" style={{ padding: '6px 16px', fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '5px' }}>
                {savingCore ? <Loader2 size={13} className="spinner" /> : <Save size={13} />} Save
              </button>
              <button onClick={() => setEditingCore(false)} className="btn-secondary" style={{ padding: '6px 16px', fontSize: '0.8rem' }}>Cancel</button>
            </div>
          </div>
        ) : (
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', minWidth: 0 }}>
                {entity.type === 'company' ? <Building2 size={16} color="var(--accent-primary)" /> : <User size={16} color="var(--accent-primary)" />}
                <span style={{ fontWeight: 700, fontSize: '0.95rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{entity.name}</span>
              </div>
              {canManage && <button onClick={startEditCore} className="btn-secondary" style={{ padding: '4px 10px', fontSize: '0.74rem', display: 'flex', alignItems: 'center', gap: '4px' }}><Pencil size={12} /> Edit</button>}
            </div>
            <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', textTransform: 'capitalize' }}>{entity.type}{entity.identifier ? ` · ${entity.identifier}` : ''}</div>
            {(entity.email || entity.phone) && (
              <div style={{ marginTop: '8px', fontSize: '0.8rem', display: 'flex', flexDirection: 'column', gap: '3px' }}>
                {entity.email && <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><Mail size={12} color="var(--text-secondary)" /><span style={{ color: 'var(--accent-primary)' }}>{entity.email}</span></div>}
                {entity.phone && <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><Phone size={12} color="var(--text-secondary)" /><span>{entity.phone}</span></div>}
              </div>
            )}
          </div>
        )}
                  ...
                  <input
                    ...
                    onBlur={e => {
                      ...
                      if (!isNaN(val)) {
                        ...
                        window.api.commissionDeleteOverride(entity.id, pt.id).then(() => {
      ...
      <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--table-border)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
          <div style={sectionLabel}>
            Addresses
            {addresses.length > 0 && <span style={{ marginLeft: '6px', padding: '1px 6px', borderRadius: '8px', background: accentBg, color: 'var(--accent-primary)', fontWeight: 700, fontSize: '0.65rem' }}>{addresses.length}</span>}
          </div>
          {canManage && <button onClick={() => { resetAddrForm(); setEditingAddress(null); setShowAddAddress(!showAddAddress) }} className="btn-secondary" style={{ padding: '3px 8px', fontSize: '0.72rem', display: 'flex', alignItems: 'center', gap: '3px' }}><Plus size={12} /> Add</button>}
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            {addresses.map(addr => (
              <div key={addr.id} style={{ padding: '8px 10px', borderRadius: '8px', background: isLight ? '#f8f9fb' : 'rgba(255,255,255,0.03)', border: '1px solid var(--table-border)', fontSize: '0.8rem' }}>
                ...
                <div style={{ color: 'var(--text-primary)', lineHeight: 1.4, whiteSpace: 'pre-line' }}>{addr.addressLine1}</div>
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{ padding: '16px 20px' }}>
          ...
          return (
            <div key={ubo.id} style={{ marginBottom: '12px', padding: '10px 12px', borderRadius: '8px', background: isLight ? 'rgba(0,0,0,0.03)' : 'rgba(255,255,255,0.03)', border: '1px solid var(--table-border)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                ...
                <span style={{ fontSize: '0.72rem', fontWeight: 600, color: uboScore.have === uboScore.total ? (isLight ? '#008c46' : '#00c264') : (isLight ? '#c00000' : '#ff4d4d') }}><Hash size={9} style={{ opacity: 0.5 }} />{uboScore.have}/{uboScore.total}</span>
                {canManage && <button onClick={() => handleDeleteUBO(ubo.id)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--text-secondary)', padding: '2px' }} className="hover-danger" title="Remove UBO"><Trash2 size={12} /></button>}
              </div>
              <OfacBadge ent={ubo} />
        ...
        {canManage && (
          <div style={{ marginTop: '8px', padding: '10px', borderRadius: '8px', border: '1px dashed var(--table-border)' }}>
                ...
                {newUBOName && !selectedUBOId && matchingUBOs.length > 0 && (
                  <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 100, marginTop: '4px', padding: '6px', maxHeight: '150px', overflowY: 'auto', background: isLight ? '#ffffff' : '#1e222a', border: '1px solid var(--accent-primary)', borderRadius: '8px', boxShadow: '0 10px 25px rgba(0,0,0,0.5)' }}>
                    {matchingUBOs.map(ent => (
                      <div key={ent.id} onClick={() => handleLinkExistingUBO(ent.id)} style={{ padding: '5px', borderRadius: '4px', cursor: 'pointer', fontSize: '0.78rem', display: 'flex', justifyContent: 'space-between' }} className="hover-effect">
            ...
            <button onClick={handleAddUBO} className="btn-primary" disabled={isAddingUBO || !newUBOName.trim()} style={{ width: '100%', padding: '6px', fontSize: '0.78rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
