...
function FilePathSettingsSection({ showSuccess, showError }: { showSuccess: (m: string) => void; showError: (m: string) => void }) {
    ...
    useEffect(() => {
        ;(async () => {
            try {
                const [s, regPath, huInfo] = await Promise.all([
    ...
    return (
        <section className="glass-card" style={{ padding: '24px', marginBottom: '32px' }}>
            ...
            <div style={{ marginTop: '28px', paddingTop: '20px', borderTop: '1px solid var(--table-border)' }}>
                ...
                <p style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', marginBottom: '12px' }}>
                    Code-only updates are delivered via GitHub automatically. The app checks for new builds every 30 minutes. No reinstall needed — just restart to apply.
                </p>
                {hotUpdateInfo && (
                    <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', fontSize: '0.78rem', color: 'var(--text-secondary)', marginBottom: '10px' }}>
                        <span>Source: <strong style={{ color: 'var(--text-primary)' }}>{hotUpdateInfo.source === 'hot-update' ? 'Hot Update' : 'Built-in'}</strong></span>
                        <span>Build: <strong style={{ color: 'var(--text-primary)' }}>{hotUpdateInfo.currentBuild || 'Base'}</strong></span>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                    <button className="btn-secondary" onClick={async () => {
                        try {
                            ...
                            if (result.updated) {
                                showSuccess(`Update downloaded (build ${result.version?.buildNumber}). Restart to apply.`)
